...
def index(request):
    ...
    # If a specific friend is requested, validate and get their chat data
    if frnd_name:
        if User.objects.filter(username=frnd_name).exists():
            ...
            # Invalid user, redirect to chat index
            return redirect('chat:index')
    
    # If no friend specified and friends exist, redirect to first friend
    elif friends_with_chats.exists():
        first_friend = friends_with_chats.first()
        return redirect(f'/chat/?user={first_friend.username}')

    # Prepare last messages for each chat
    last_messages = {}
    for friend in friends_with_chats:
        try:
            ...
            if chat and chat.chats:
                # Assuming chats is a dict with timestamp keys
                latest_key = max(chat.chats.keys())
                last_messages[friend.username] = chat.chats[latest_key]['msg']
        except (ValueError, TypeError, KeyError) as e:
            last_messages[friend.username] = "Start a conversation..."
